...
class PolygonOut extends FormatAbstract
{
    /**
     * @return string
     */
    ...
    public function validate(): void
    {
        ...
        if (empty($config['latitude'])) {
            $this->validateException(__('alarm-type-polygon-out.error.latitude'));
        }

        if (empty($config['longitude'])) {
            $this->validateException(__('alarm-type-polygon-out.error.longitude'));
        }

        if (empty($config['radius'])) {
            $this->validateException(__('alarm-type-polygon-out.error.radius'));
        }
    }
