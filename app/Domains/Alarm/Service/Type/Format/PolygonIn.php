...
class PolygonIn extends FormatAbstract
{
    /**
     * @return string
     */
    ...
    public function validate(): void
    {
        ...
        if (empty($config['latitude'])) {
            $this->validateException(__('alarm-type-polygon-in.error.latitude'));
        }

        if (empty($config['longitude'])) {
            $this->validateException(__('alarm-type-polygon-in.error.longitude'));
        }

        if (empty($config['radius'])) {
            $this->validateException(__('alarm-type-polygon-in.error.radius'));
        }
    }
