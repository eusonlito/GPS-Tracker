...
<form method="get">
    <div class="sm:flex sm:space-x-4">
        ...
        <div class="sm:ml-4 mt-2 sm:mt-0 bg-white">
            <a href="{{ route('role.create') }}" class="btn form-control-lg whitespace-nowrap">{{ __('role-index.create') }}</a>
        </div>
    </div>
</form>
...
<div class="overflow-auto scroll-visible header-sticky">
    <table id="role-list-table" class="table table-report sm:mt-2 font-medium font-semibold text-center whitespace-nowrap" data-table-sort data-table-pagination data-table-pagination-limit="10">
        <thead>
            <tr>
                @if ($user_empty)
                <th>{{ __('role-index.user') }}</th>
                @endif

                <th class="w-1">{{ __('role-index.type') }}</th>
                <th class="text-left w-1">{{ __('role-index.name') }}</th>
                <th class="w-1">{{ __('role-index.vehicles') }}</th>
                <th class="w-1">{{ __('role-index.notifications') }}</th>
                <th class="w-1">{{ __('role-index.created_at') }}</th>
                <th class="w-1">{{ __('role-index.telegram') }}</th>
                <th class="w-1">{{ __('role-index.enabled') }}</th>
            </tr>
        </thead>
        ...
        <tbody>
            ...
            @php ($link = route('role.update', $row->id))
            ...
            <tr>
                ...
                <td class="w-1"><a href="{{ route('role.update.vehicle', $row->id) }}">{{ $row->vehicles_count }}</a></td>
                <td class="w-1">
                    <a href="{{ route('role.update.role-notification', $row->id) }}" class="{{ $row->notifications_pending_count ? 'text-warning' : 'text-success' }}">
                ...
                <td class="w-1" data-table-sort-value="{{ intval($row->telegram) }}"><a href="{{ route('role.update.boolean', [$row->id, 'telegram']) }}" class="block" data-update-boolean="telegram">@status($row->telegram)</a></td>
                <td class="w-1" data-table-sort-value="{{ intval($row->enabled) }}"><a href="{{ route('role.update.boolean', [$row->id, 'enabled']) }}" class="block" data-update-boolean="enabled">@status($row->enabled)</a></td>
            </tr>
