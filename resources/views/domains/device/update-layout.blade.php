...
<div class="box flex items-center px-5">
    <div class="nav nav-tabs flex overflow-auto whitespace-nowrap" role="tablist">
        ...
        <a href="{{ route('alarm', $row->id) }}" class="p-4 {{ (($ROUTE === 'alarm') || str_starts_with($ROUTE, 'alarm.')) ? 'active' : '' }}" role="tab">{{ __('device-update.alarms') }}</a>
        <a href="{{ route('alarm-notification', $row->id) }}" class="p-4 {{ str_starts_with($ROUTE, 'alarm-notification') ? 'active' : '' }}" role="tab">{{ __('device-update.notifications') }}</a>
