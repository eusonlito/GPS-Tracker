...
<div class="box flex items-center px-5">
    <div class="nav nav-tabs flex overflow-auto whitespace-nowrap" role="tablist">
        ...
        <a href="{{ route('device.update.alarm', $row->id) }}" class="p-4 {{ ($ROUTE === 'device.update.alarm') ? 'active' : '' }}" role="tab">{{ __('device-update.alarms') }}</a>
        <a href="{{ route('device.update.alarm-notification', $row->id) }}" class="p-4 {{ ($ROUTE === 'device.update.alarm-notification') ? 'active' : '' }}" role="tab">{{ __('device-update.notifications') }}</a>
