...
<div class="box flex items-center px-5">
    <div class="nav nav-tabs flex overflow-auto whitespace-nowrap" role="tablist">
        <a href="{{ route('configuration.update', $row->id) }}" class="p-4 {{ ($ROUTE === 'configuration.update') ? 'active' : '' }}" role="tab">{{ $row->key }}</a>
    </div>
</div>
...
<div class="box p-5 mt-5">
    <div class="p-2">
        <form method="get">
            <x-select name="user_id" :options="$users" value="id" text="name" id="device-create-user" :label="__('device-create.user')" :selected="$REQUEST->input('user_id')" data-change-submit required></x-select>
        </form>
    </div>
</div>
...
<div class="tab-content">
    <div class="tab-pane active" role="tabpanel">
        <form method="post">
            ...
            <div class="box p-5 mt-5">
                <div class="text-right">
                    <button type="submit" class="btn btn-primary">{{ __('ip-lock-create.save') }}</button>
                </div>
            </div>
        </form>
    </div>
...
@include ('domains.monitor.log.header')
...
<div class="flex mb-2 mb-5 border-b-2">
    <h2 class="text-lg font-medium"><a href="{{ route('monitor.log.path') }}">@icon('home', 'w-5 h-5')</a></h2>
    ...
    <table id="monitor-log-table" class="table table-report sm:mt-2 font-medium font-semibold text-center whitespace-nowrap" data-table-sort data-table-pagination data-table-pagination-limit="10">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    @if ($each->route_download)
                    <a href="{{ $each->route_download }}">@icon('download', 'w-4 h-4')</a>
                    @endif
                </td>
            </tr>
...
<form method="get">
    <div class="lg:flex lg:space-x-4">
        ...
        <div class="flex-grow mt-2 lg:mt-0">
            <input type="search" name="start_at" value="{{ $REQUEST->input('start_at') }}" class="form-control form-control-lg" placeholder="{{ __('refuel-index.start_at') }}" data-datepicker data-datepicker-min-date="{{ $date_min }}" data-change-submit />
        </div>

        <div class="flex-grow mt-2 lg:mt-0">
            <input type="search" name="end_at" value="{{ $REQUEST->input('end_at') }}" class="form-control form-control-lg" placeholder="{{ __('refuel-index.end_at') }}" data-datepicker data-datepicker-min-date="{{ $date_min }}" data-change-submit />
        </div>
            ...
            <table id="server-status-list-table" class="table table-report sm:mt-2 font-medium font-semibold text-center whitespace-nowrap" data-table-sort data-table-pagination data-table-pagination-limit="10">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><a href="{{ $link }}" class="block">{{ $row->protocol }}</a></td>
                        <td class="w-1" data-table-sort-value="{{ (int)$row->debug }}"><a href="{{ route('server.update.boolean', [$row->id, 'debug']) }}" class="block" data-update-boolean="debug">@status($row->debug)</a></td>
                        <td class="w-1" data-table-sort-value="{{ (int)$row->enabled }}"><a href="{{ route('server.update.boolean', [$row->id, 'enabled']) }}" class="block">@status($row->enabled)</a></td>
            ...
            <table id="state-update-merge-table" class="table table-report font-medium font-semibold text-center whitespace-nowrap" data-table-sort data-table-pagination data-table-pagination-limit="10">
                <thead>
                    <tr>
                        ...
                        <th>{{ __('state-update-merge.created_at') }}</th>
                        <th>{{ __('state-update-merge.updated_at') }}</th>
...
<form id="trip-heatmap-form" method="get">
    <div class="lg:flex lg:space-x-4">
        ...
        <div class="flex-grow mt-2 lg:mt-0">
            <input type="search" name="start_at" value="{{ $REQUEST->input('start_at') }}" class="form-control form-control-lg" placeholder="{{ __('trip-index.start_at') }}" data-datepicker data-datepicker-min-date="{{ $date_min }}" data-change-submit />
        </div>

        <div class="flex-grow mt-2 lg:mt-0">
            <input type="search" name="end_at" value="{{ $REQUEST->input('end_at') }}" class="form-control form-control-lg" placeholder="{{ __('trip-index.end_at') }}" data-datepicker data-datepicker-min-date="{{ $date_min }}" data-change-submit />
        </div>
...
<form id="trip-map-form" class="mb-5" method="get">
    <div class="lg:flex lg:space-x-4">
        ...
        <div class="flex-grow mt-2 lg:mt-0">
            <input type="search" name="start_at" value="{{ $REQUEST->input('start_at') }}" class="form-control form-control-lg" placeholder="{{ __('trip-map.start_at') }}" data-datepicker data-datepicker-min-date="{{ $date_min }}" data-change-submit />
        </div>

        <div class="flex-grow mt-2 lg:mt-0">
            <input type="search" name="end_at" value="{{ $REQUEST->input('end_at') }}" class="form-control form-control-lg" placeholder="{{ __('trip-map.end_at') }}" data-datepicker data-datepicker-min-date="{{ $date_min }}" data-change-submit />
        </div>
...
<html lang="{{ app()->getLocale() }}">
    ...
    <body class="main">
        <div class="container">
            <div class="error-page flex flex-col lg:flex-row items-center justify-center h-screen text-center lg:text-left">
                <div class="lg:mr-20">
                    <img class="h-48 lg:h-auto" src="@asset('build/images/error-illustration.svg')">
                </div>

                <div class="text-white mt-10 lg:mt-0">
                    <div class="text-4xl font-medium">{{ __('user-disabled.message') }}</div>
                    <a href="{{ route('user.auth.credentials') }}" class="btn py-3 px-4 text-white border-white mt-10">{{ __('error.back-home') }}</a>
                </div>
            </div>
        </div>
    ...
    <table id="alarm-notification-list-table" class="table table-report sm:mt-2 font-medium font-semibold text-center whitespace-nowrap" data-table-sort data-table-pagination data-table-pagination-limit="10">
        <thead>
            <tr>
                ...
                <th class="w-1">{{ __('vehicle-update-alarm-notification.dashboard') }}</th>
...
```php
<div class="lg:h-3/4 flex flex-col items-center justify-center">
    <img class="mt-10 lg:mt-0 h-24 lg:h-48" src="@asset('build/images/empty.svg')">
    <h3 class="text-lg font-medium text-slate-900 mt-10">{{ __('common.content-empty')}}</h3>
</div>
